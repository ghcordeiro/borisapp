    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="nutrition">
              ...
              <CardContent className="pt-4 pb-3">
                ...
                <div className="space-y-1.5 max-h-48 overflow-y-auto">
                  {[...serialized.weightLogs].reverse().map((log, i) => (
                    <div key={log.id} className="flex items-center justify-between text-sm py-1 border-b border-border/40 last:border-0">
                      <div className="flex items-center gap-2">
                        <span className="text-muted-foreground text-xs w-5 text-right">{i === 0 ? "🔴" : ""}</span>
                      ...
                      <div className="flex items-center gap-3">
                        {log.notes && (
                          <span className="text-xs text-muted-foreground italic truncate max-w-[140px]">{log.notes}</span>
                        )}
        ...
        <TabsContent value="health" className="space-y-4 pt-4">
          ...
          {canMutate && (
            <div className="flex flex-wrap gap-2">
              <AddHealthLogForm petId={serialized.id} />
              <AddVetAppointmentForm petId={serialized.id} />
              <AddDewormingForm petId={serialized.id} />
              <AddVaccineForm petId={serialized.id} />
            </div>
          )}
          ...
          <VaccineList vaccines={serialized.vaccines} />
